...
// formCloudflareImage is idle while the banners are local files — the import
// above stays for the legacy slide set below.
const { formCloudflareImage } = formatStuff();
void formCloudflareImage;
...
// Previous Cloudflare-hosted slides. Disabled, not deleted — these carry the
// image IDs and the game/route each banner used to open.
// const legacyItems = [
...
function launchGame(gameId, gameName, routeName) {
  if (ROUTE_ALL_BANNERS_HOME) { router.push({ name: "home" }); return; }
    ...
    <div class="relative w-full overflow-clip">
      ...
      >
        <swiper-slide v-for="item in items" :key="item.image">
          <img
            :src="item.image"
            class="w-full aspect-[3/1] cursor-pointer object-cover"
